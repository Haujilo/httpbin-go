...
func TestDeflateHandler(t *testing.T) {
	...
	createTestCase := func(name string) struct {
		name   string
		args   args
		result deflateJSONResponse
	} {
		r, err := http.NewRequest("GET", "/deflate", nil)
		if err != nil {
			t.Fatal(err)
		}
		return struct {
			...
			args   args
			result deflateJSONResponse
		}{name, args{httptest.NewRecorder(), r}, deflateJSONResponse{true, fmtHeaders(r), r.Method, getIP(r)}}
	}
	tests := []struct {
		name   string
		args   args
		...
		createTestCase("TestDeflateHandler1"),
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			DeflateHandler(tt.args.w, tt.args.r)
			if status := tt.args.w.Code; status != http.StatusOK {
				t.Errorf("handler returned wrong status code: got %v want %v",
					status, http.StatusOK)
			}
			reader := flate.NewReader(bytes.NewReader(tt.args.w.Body.Bytes()))
			defer reader.Close()
			content, err := ioutil.ReadAll(reader)
			if err != nil {
				t.Error(err)
			}
			var body deflateJSONResponse
			json.Unmarshal(content, &body)
			if !reflect.DeepEqual(tt.result, body) {
				t.Errorf("handler returned wrong response json body: got %v want %v",
					body, tt.result)
...
func TestUTF8Handler(t *testing.T) {
	type args struct {
		w *httptest.ResponseRecorder
		r *http.Request
	}
	createTestCase := func() args {
		r, err := http.NewRequest("GET", "/encoding/utf8", nil)
		if err != nil {
			t.Fatal(err)
		}
		return args{httptest.NewRecorder(), r}
	...
	tests := []struct {
		...
		{"TestUTF8Handler1", createTestCase(), utf8Demo},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			UTF8Handler(tt.args.w, tt.args.r)
			if status := tt.args.w.Code; status != http.StatusOK {
				t.Errorf("handler returned wrong status code: got %v want %v",
					status, http.StatusOK)
			}
...
func TestDenyHandler(t *testing.T) {
	...
	tests := []struct {
		...
		{"TestDenyHandler1", createTestCase(), angryAscii},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			DenyHandler(tt.args.w, tt.args.r)
		})
		if status := tt.args.w.Code; status != http.StatusOK {
			t.Errorf("handler returned wrong status code: got %v want %v",
				status, http.StatusOK)
