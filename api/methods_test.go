...
func TestPATCHHandler(t *testing.T) {
	...
	createTestCase := func(host, path, addr string, headers [][2]string, body io.Reader) args {
		r, err := http.NewRequest("PATCH", path, body)
		if err != nil {
			t.Fatal(err)
		}
		r.RequestURI = path
	...
	tests := []struct {
		...
		{
			"TestPATCHHandler1",
			createTestCase("localhost:1121", "/patch?a=1&a=2&b=3", "127.0.0.1:1121", [][2]string{{"Content-Type", "application/x-www-form-urlencoded"}, {"X-TEST", "Test"}}, func() io.Reader {
				data := url.Values{}
				data.Add("test1", "test1")
				data.Add("test1", "test1-1")
				data.Add("test2", "test2")
			...
			methodsJSONResponse{
				...
				Headers: map[string]string{"Content-Type": "application/x-www-form-urlencoded", "X-Test": "Test"},
				Origin:  "127.0.0.1",
				URL:     "http://localhost:1121/patch?a=1\u0026a=2\u0026b=3",
				Form: map[string]interface{}{
					"test1": []interface{}{"test1", "test1-1"},
					"test2": "test2",
				},
		...
		},
		{
			...
			func() args {
				...
				ww, _ := w.CreateFormFile("testfile", "testfile")
				ww.Write([]byte("abc\n"))
				return createTestCase("localhost:1121", "/patch?a=1&a=2&b=3", "127.0.0.1:1121", [][2]string{{"Content-Type", w.FormDataContentType()}, {"X-TEST", "Test"}}, &body)
			}(),
			methodsJSONResponse{
				Args: map[string]interface{}{
					"a": []interface{}{"1", "2"},
		...
		{
			"TestPATCHHandler3",
			createTestCase("localhost:1121", "/patch?a=1&a=2&b=3", "127.0.0.1:1121", [][2]string{{"Content-Type", "application/json"}, {"X-TEST", "Test"}}, strings.NewReader("{\"a\": \"1\"}")),
			methodsJSONResponse{
				Args: map[string]interface{}{
					"a": []interface{}{"1", "2"},
					"b": "3",
		...
		{
			"TestPATCHHandler4",
			createTestCase("localhost:1121", "/patch?a=1&a=2&b=3", "127.0.0.1:1121", [][2]string{{"X-TEST", "Test"}}, strings.NewReader("abcdefgh")),
			methodsJSONResponse{
				Args: map[string]interface{}{
					"a": []interface{}{"1", "2"},
					"b": "3",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			PATCHHandler(tt.args.w, tt.args.r)
			if status := tt.args.w.Code; status != http.StatusOK {
				t.Errorf("handler returned wrong status code: got %v want %v",
					status, http.StatusOK)
			}
...
func TestDELETEHandler(t *testing.T) {
	...
	createTestCase := func(host, path, addr string, headers [][2]string, body io.Reader) args {
		r, err := http.NewRequest("DELETE", path, body)
		if err != nil {
			t.Fatal(err)
		}
		r.RequestURI = path
	...
	tests := []struct {
		...
		{
			"TestDELETEHandler1",
			createTestCase("localhost:1121", "/delete?a=1&a=2&b=3", "127.0.0.1:1121", [][2]string{{"Content-Type", "application/x-www-form-urlencoded"}, {"X-TEST", "Test"}}, func() io.Reader {
				data := url.Values{}
				data.Add("test1", "test1")
				data.Add("test1", "test1-1")
				data.Add("test2", "test2")
			...
			methodsJSONResponse{
				...
				Headers: map[string]string{"Content-Type": "application/x-www-form-urlencoded", "X-Test": "Test"},
				Origin:  "127.0.0.1",
				URL:     "http://localhost:1121/delete?a=1\u0026a=2\u0026b=3",
		...
		},
		{
			...
			func() args {
				...
				ww, _ := w.CreateFormFile("testfile", "testfile")
				ww.Write([]byte("abc\n"))
				return createTestCase("localhost:1121", "/delete?a=1&a=2&b=3", "127.0.0.1:1121", [][2]string{{"Content-Type", w.FormDataContentType()}, {"X-TEST", "Test"}}, &body)
			}(),
			methodsJSONResponse{
				Args: map[string]interface{}{
					"a": []interface{}{"1", "2"},
		...
		{
			"TestDELETEHandler3",
			createTestCase("localhost:1121", "/delete?a=1&a=2&b=3", "127.0.0.1:1121", [][2]string{{"Content-Type", "application/json"}, {"X-TEST", "Test"}}, strings.NewReader("{\"a\": \"1\"}")),
			methodsJSONResponse{
				Args: map[string]interface{}{
					"a": []interface{}{"1", "2"},
					"b": "3",
		...
		{
			"TestDELETEHandler4",
			createTestCase("localhost:1121", "/delete?a=1&a=2&b=3", "127.0.0.1:1121", [][2]string{{"X-TEST", "Test"}}, strings.NewReader("abcdefgh")),
			methodsJSONResponse{
				Args: map[string]interface{}{
					"a": []interface{}{"1", "2"},
					"b": "3",
